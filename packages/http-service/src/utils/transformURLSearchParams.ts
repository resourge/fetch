...
const transformURLSearchParamsFromArray = <K extends any[]>(
	...
	params.map(([_key, value]) => [`${baseKey}[]`, value])
);
...
const transformURLSearchParamsFromObject = <K extends object>(
	...
	return transformURLSearchParams(
		Object.entries(params).map(([key, value]) => [`${_baseKey}${key}`, value])
	);
};
 ...
 */
export const convertParamsToQueryString = <K extends any[] | object>(params: K) => {
